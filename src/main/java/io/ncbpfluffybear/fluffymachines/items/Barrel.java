...
public class Barrel extends NonHopperableBlock implements DoubleHologramOwner {
    ...
    private final ItemStack HOLOGRAM_OFF_ITEM = new CustomItemStack(Material.QUARTZ_SLAB, "&3切换全息显示 &c(关闭)");
    private final ItemStack HOLOGRAM_ON_ITEM = new CustomItemStack(Material.QUARTZ_SLAB, "&3切换全息显示 &a(打开)");
    private final ItemStack TRASH_ON_ITEM = new CustomItemStack(SlimefunItems.TRASH_CAN, "&3满载时清空输入 &a(打开)",
            "&7开启时清空满载后的输入");
    private final ItemStack TRASH_OFF_ITEM = new CustomItemStack(SlimefunItems.TRASH_CAN, "&3满载时清空输入 &c(关闭)",
            "&7开启时清空满载后的输入"
    );
        ...
        new BlockMenuPreset(getId(), name) {
            ...
            public void newInstance(@Nonnull BlockMenu menu, @Nonnull Block b) {
                ...
                if (BlockStorage.getLocationInfo(b.getLocation(), "stored") == null) {
                    ...
                    menu.replaceExistingItem(DISPLAY_SLOT, new CustomItemStack(Material.BARRIER, "&c空的"));

                    setStored(b, 0);

                    if (showHologram.getValue()) {
                        updateHologram(b, null, "&c空的");
                    }
                ...
                menu.replaceExistingItem(INSERT_ALL_SLOT,
                    new CustomItemStack(Material.LIME_STAINED_GLASS_PANE, "&b放入所有",
                        "&7> 点击此处将所有可用物品放入蓬松桶"));
                ...
                menu.replaceExistingItem(EXTRACT_ALL_SLOT,
                    new CustomItemStack(Material.RED_STAINED_GLASS_PANE, "&6取出所有",
                        "&7> 点击此处取出所有物品"));
    ...
    public void updateMenu(Block b, BlockMenu inv, boolean needsViewer) {
        ...
        if (stored == 0) {
            inv.replaceExistingItem(DISPLAY_SLOT, new CustomItemStack(Material.BARRIER, "&c空的"));
            if (showHologram.getValue() && (hasHolo == null || hasHolo.equals("true"))) {
                updateHologram(b, null, "&c空的");
            }
        }
